 ...
 */
function handleIntegerInput(input, errorId, maxLength) {
    // Remove non-digit characters and limit length
    input.value = input.value.replace(/\D/g, '').slice(0, maxLength);
    document.getElementById(errorId).textContent = '';
}

function validateIntegerInput(input, errorId, maxLength) {
    const value = input.value.trim();
    if (!/^\d+$/.test(value) || value.length > maxLength) {
        document.getElementById(errorId).textContent = "Only numeric values allowed (max " + maxLength + " digits)";
    } else {
        document.getElementById(errorId).textContent = '';
    }
}
...
*/
/*function validateAlphanumeric(input, errorId, maxLength) {
    ...
    if (value !== cleaned) {
        document.getElementById(errorId).textContent = "Only alphanumeric characters allowed";
    } else if (cleaned.length > maxLength) {
        document.getElementById(errorId).textContent = "Maximum " + maxLength + " characters allowed";
    } else {
        document.getElementById(errorId).textContent = "";
    }

    // Apply the cleaned value (trimmed to maxLength)
...
function handleAlphanumeric(inputElement, errorElementId) {
    const errorMessage = document.getElementById(errorElementId);
    const currentValue = inputElement.value;
    ...
    if (/[^a-zA-Z0-9]/.test(currentValue)) {
        ...
        errorMessage.textContent = 'Input must not exceed 15 characters.';
        inputElement.value = currentValue.slice(0, 15);
    } else if (currentValue.length < 5) {
        // Check for minimum length
        errorMessage.textContent = 'Input must be at least 5 characters long.';
    } else {
...
function validateAlphanumericInput1(inputElement, errorElementId, minLength, maxLength) {
    const errorMessage = document.getElementById(errorElementId);
    const currentValue = inputElement.value;

    // Check for minimum length
    if (currentValue.length < minLength) {
        errorMessage.textContent = `Input must be at least ${minLength} characters long.`;
    } 
    // Check for maximum length
