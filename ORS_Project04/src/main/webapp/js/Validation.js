...
function validateMobile(event) {
    ...
    if (input.value.length > 0 && !/^[6789]/.test(input.value.charAt(0))) {
        input.value = ''; // Clear the input if it starts with any digit other than 6, 7, 8, or 9
        return;
    }

    // Check if the second digit is 0 or 1
    if (input.value.length > 1 && /^[01]/.test(input.value.charAt(1))) {
        input.value = input.value.charAt(0); // Keep only the first digit if the second digit is 0 or 1
    }

    // Uncomment this block if you want to limit the input to 10 digits
    /*
